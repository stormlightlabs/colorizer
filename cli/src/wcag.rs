...
mod tests {
    ...
    fn test_relative_luminance_colors() {
        ...
        assert!(
            lum_g > lum_b,
            "Green ({}) should be brighter than blue ({})",
            lum_g,
            lum_b
        );
        assert!(
    ...
    fn test_wcag_aa_normal_threshold() {
        ...
        let ratio = contrast_ratio(white, dark_gray);
        assert!(ratio >= 4.3 && ratio <= 4.7, "Ratio: {}", ratio);
    }
    ...
    fn test_wcag_aa_large_threshold() {
        ...
        assert!(meets_aa_large(ratio), "Ratio {} should meet AA large (3:1)", ratio);
        assert!(
            !meets_aa_normal(ratio),
            "Ratio {} should not meet AA normal (4.5:1)",
            ratio
        );
    }
    ...
    fn test_wcag_aaa_normal_threshold() {
        ...
        assert!(ratio >= 6.8 && ratio <= 7.5, "Ratio: {}", ratio);
    }
    ...
    fn test_wcag_known_reference_values() {
        ...
        assert!(ratio >= 4.4 && ratio <= 4.6, "Ratio: {}", ratio);
